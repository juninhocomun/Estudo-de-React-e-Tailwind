...
function Clover({
  ...
  const [cloverContainerAnimation, setCloverContainerAnimation] =
    useState(toCorner);
  const [cloverLeaftAnimation, setCloverLeaftAnimation] = useState(false);
  function handleMoveToCorner() {
    setCloverContainerAnimation(!cloverContainerAnimation);
    setCloverLeaftAnimation(true);
  }
  useEffect(() => {
    setCloverLeaftAnimation(animation);
  }, [animation]);
  return (
    <div
      className={` ${
        cloverContainerAnimation
          ? "clover-container to-corner"
          : `clover-container ${size} `
      }`}
      id="clover-container"
    >
      {debugButton ? (
        <button onClick={handleMoveToCorner}> Mover</button>
      ) : null}
      <div
        className={`${
          cloverLeaftAnimation || !animation
            ? "clover-leafs-container animation"
            : "clover-leafs-container"
        } animate-spin`}
        style={{ animationDuration: speed }}
      >
        {[...Array(5).keys()].map((index) => (
          <Leaf key={index} animation={animation} />
        ))}
      </div>
    </div>
  );
...
function Leaf({ animation }) {
  return (
    <svg
      className={`clover ${animation ? "" : "no-animation"}`}
      style={{
